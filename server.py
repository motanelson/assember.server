...
<html>
<head>
    <title>ASM Compiler Server</title>
    <style>
        body {
...
<body>
    <h1>Upload a ASM File</h1>
    <form method="post" action="/upload" enctype="multipart/form-data">
        <input type="file" name="cfile" accept=".asm" required />
...
def upload_file():
    ...
    if not file.filename.endswith('.asm'):
        return "Only .asm files are allowed", 400

    # Salvar o arquivo
    c_file_path = os.path.join(UPLOAD_FOLDER, str(file_counters)+".asm")
